...
public class PaymentInstrumentController {
	...
	@Transactional
	public String unlockPaymentInstrument(@RequestParam Long paymentInstrumentId,
			RedirectAttributes redirectAttributes) {
		paymentInstrumentService.unlockPaymentInstrument(paymentInstrumentId);
		redirectAttributes.addFlashAttribute("message",
				String.format("Payment instrument %s activated.", paymentInstrumentId));
		return "redirect:/";
	}

	@RequestMapping(value = "/createBankAccount", method = RequestMethod.POST)
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	public String createBankAccount(@RequestParam String accountHolder, @RequestParam String bankName,
			@RequestParam String iban, @RequestParam String bic, RedirectAttributes redirectAttributes) {
		long bankAccountId = paymentInstrumentService.createBankAccount(accountHolder, bankName, iban, bic);
		redirectAttributes.addFlashAttribute("message",
				String.format("Bank account with ID %s created.", bankAccountId));
		return "redirect:/";
	}

	@RequestMapping(value = "/createCreditCard", method = RequestMethod.POST)
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	public String createCreditCard(@RequestParam String accountHolder, @RequestParam CreditCardScheme scheme,
			@RequestParam String pan, @RequestParam Integer expiryMonth, @RequestParam Integer expiryYear,
			RedirectAttributes redirectAttributes) {
		long creditCardId = paymentInstrumentService.createCreditCard(accountHolder, scheme, pan, expiryMonth,
				expiryYear);
